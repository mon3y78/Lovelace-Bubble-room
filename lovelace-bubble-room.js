...
`;class Ft extends st{static properties={hass:{type:Object},config:{type:Object},expanded:{type:Boolean},_expanded:{type:Array,state:!0},_filters:{type:Array,state:!0},_entities:{type:Array,state:!0}};constructor(){super(),this.hass={},this.config={},this.expanded=!1,this._expanded=Array(5).fill(!1);const t=Object.keys(zt);this._filters=Array(5).fill().map(()=>[...t]),this._entities=Array(5).fill(""),this._ignoreNextFilterChange=new Set}updated(t){if(t.has("config")||t.has("hass")){const t=_t(this.hass,this.config,"auto_discovery_sections.sensor");t&&t!==this.config&&this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}));for(let t=0;t<5;t++){const e=`sensor${t+1}`,i=this.config?.sensor_filters?.[t],s=this.config?.entities?.[e]?.entity;Array.isArray(i)&&(this._filters[t]=[...i]),s&&(this._entities[t]=s)}}}static styles=[Pt,o`
    ...
    `];render(){const t=this.config?.auto_discovery_sections?.sensor??!1,e=Object.entries(zt).filter(([t])=>"_fallback"!==t).map(([t,e])=>{const i=e.label||t.replace(/_/g," ").replace(/\b\w/g,t=>t.toUpperCase());return{value:t,label:`${e.emoji||""} ${i}`.trim()}});return R`
      <ha-expansion-panel
        class="glass-panel"
      ...
      <div class="mini-pill ${e?"expanded":""}">
        ...
        ${e?R`
          <div class="mini-pill-content">
            ...
            ${n?(()=>{const t=this.hass.states[n],e=t?.attributes?.device_class,i=zt[e]||{},s=i.emoji||"❓",o=t?.attributes?.unit_of_measurement||(i.units?.[0]??"");return R`
                <div class="preview">
    ...
    `];render(){const t=this.config?.auto_discovery_sections?.mushroom??!1,e=this._ALL_CATS.map(t=>({value:t,label:ot[t]||t.charAt(0).toUpperCase()+t.slice(1)}));return R`
      <ha-expansion-panel
        class="glass-panel"
        .expanded=${this.expanded}
        @expanded-changed=${t=>{this.expanded=t.detail.expanded,this.expanded&&(this._expanded=Array(5).fill(!1))}}
  ...
  `;render(){return R`
    ...
    `}_toggleColor(t){this._expandedColors=this._expandedColors.map((e,i)=>i===t&&!e)}_renderColorField(t,e,i){const s=this.config?.colors?.[t]?.[e]||"",[n,o,a,r]=this._parseRGBA(s),l=`#${[n,o,a].map(t=>t.toString(16).padStart(2,"0")).join("")}`;return R`
      <div class="input-group">
        <label>${i}</label>
        <input
          type="color"
  ...
  `;render(){const t=this.config||{};return R`
    ...
    `}}customElements.define("bubble-room-editor",Rt);var It=Object.freeze({__proto__:null,BubbleRoomEditor:Rt});class Ut extends st{static properties={subbuttons:{type:Array}};constructor(){super(),this.subbuttons=[],this._holdThreshold=500,this._holdTimer=null,this._holdFired=!1,this._currentIndex=-1}static styles=o`
    :host {
      display: block;
      height: 100%;
      width: 100%;
  ...
  `;render(){const t=(this.sensors||[]).map(t=>{const e=t.device_class,i=zt[e]||{},s=i.emoji||"❓",n=t.unit||i.units?.[0]||"",o=this._formatValueForDisplay(t.value,1,!0);return{...t,value:o,label:s,unit:n}});return R`
      <div
        class="sensor-grid"
        style="
        ...
        "
      >
        ${t.map(t=>{const e=t.entity||t.entity_id||"",i=e?`Show history graph: ${e}`:"Show history graph";return R`
            <div
              class="sensor-pill"
  ...
  `;render(){const{width:t,height:e}=this._containerSize;if(!t||!e)return R``;const i=window.innerWidth||t,s=.55;let n;if(i<=100)n=s;else if(i>=200)n=.25;else{n=s+(.25-s)*((i-100)/100)}const o=Math.min(t,1.6*e),a=.5*(e+o)*n,r=.6*t,l=.6*e,c=r*Math.min(1,t/(2*r)),d=l*Math.min(1,e/(2*l)),h=t-c,u=.5*e,p=Math.max(0,c-a/2-1),b=Math.max(0,d-a/2-1),g=t=>Math.PI*t/180,f=g(30),m=g(85),_=.75*a,v=.75*a,y=[{x:a/2+1,y:a/2+1},{x:h+p*Math.cos(-m),y:u+b*Math.sin(-m)},{x:h+p*Math.cos(-f),y:u+b*Math.sin(-f)},{x:h+p*Math.cos(+f),y:u+b*Math.sin(+f)},{x:h+p*Math.cos(+m),y:u+b*Math.sin(+m)}];let x=0;return R`
      ${this.entities.map(i=>{const s="camera"===i?.kind,n="climate"===i?.kind,o=s?_:n?v:a,r=.95*o;let l;s?l={x:t-o/2,y:o/2}:n?l={x:o/2+1,y:e-o/2-1}:(l=y[Math.min(x,y.length-1)]??{x:h,y:u},x++);const c=s||n?0:i.dx??0,d=s||n?0:i.dy??0,p=l.x+c,b=l.y+d;return R`
          <div
  ...
  `;render(){const t=this.active?this.colorActive:this.colorInactive,e=this.active?this.backgroundActive:this.backgroundInactive,i=this._withOpacity(e,.1)??e,s=this.active?.9:.8;return R`
      <div
        class="container"
        style="background:${i}"
