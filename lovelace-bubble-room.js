...
`;class Ft extends st{static properties={hass:{type:Object},config:{type:Object},expanded:{type:Boolean},_expanded:{type:Array,state:!0},_filters:{type:Array,state:!0},_entities:{type:Array,state:!0}};constructor(){super(),this.hass={},this.config={},this.expanded=!1,this._expanded=Array(5).fill(!1);const t=Object.keys(zt);this._filters=Array(5).fill().map(()=>[...t]),this._entities=Array(5).fill(""),this._ignoreNextFilterChange=new Set}updated(t){if(t.has("config")||t.has("hass")){const t=_t(this.hass,this.config,"auto_discovery_sections.sensor");t&&t!==this.config&&this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}));for(let t=0;t<5;t++){const e=`sensor${t+1}`,i=this.config?.sensor_filters?.[t],s=this.config?.entities?.[e]?.entity;Array.isArray(i)&&(this._filters[t]=[...i]),s&&(this._entities[t]=s)}}}static styles=[Pt,o`
    ...
    `];render(){const t=this.config?.auto_discovery_sections?.mushroom??!1,e=this._ALL_CATS.map(t=>({value:t,label:ot[t]||t.charAt(0).toUpperCase()+t.slice(1)}));return N`
      <ha-expansion-panel
        class="glass-panel"
        .expanded=${this.expanded}
        @expanded-changed=${t=>{this.expanded=t.detail.expanded,this.expanded&&(this._expanded=Array(5).fill(!1))}}
  ...
  `;render(){return N`
      <div class="container">
        ${this.subbuttons.map((t,e)=>{const i=t.active?t.colorOn:t.colorOff,s=t.active?t.iconOn:t.iconOff;return N`
            <div
              class="sub-button"
              style="--bubble-subbutton-bg:${i};--bubble-subbutton-color:${s};"
  ...
  `;render(){const{width:t,height:e}=this._containerSize;if(!t||!e)return N``;const i=window.innerWidth||t,s=.55;let n;if(i<=100)n=s;else if(i>=200)n=.25;else{n=s+(.25-s)*((i-100)/100)}const o=Math.min(t,1.6*e),a=.5*(e+o)*n,r=.6*t,c=.6*e,l=r*Math.min(1,t/(2*r)),d=c*Math.min(1,e/(2*c)),h=t-l,u=.5*e,p=Math.max(0,l-a/2-1),b=Math.max(0,d-a/2-1),g=t=>Math.PI*t/180,f=g(30),m=g(85),_=.75*a,v=.75*a,y=[{x:a/2+1,y:a/2+1},{x:h+p*Math.cos(-m),y:u+b*Math.sin(-m)},{x:h+p*Math.cos(-f),y:u+b*Math.sin(-f)},{x:h+p*Math.cos(+f),y:u+b*Math.sin(+f)},{x:h+p*Math.cos(+m),y:u+b*Math.sin(+m)}];let x=0;return N`
      ${this.entities.map(i=>{const s="camera"===i?.kind,n="climate"===i?.kind,o=s?_:n?v:a,r=.95*o;let c;s?c={x:t-o/2,y:o/2}:n?c={x:o/2+1,y:e-o/2-1}:(c=y[Math.min(x,y.length-1)]??{x:h,y:u},x++);const l=s||n?0:i.dx??0,d=s||n?0:i.dy??0,p=c.x+l,b=c.y+d;return N`
          <div
