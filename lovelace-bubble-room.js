...
`;class Oe extends ie{static properties={hass:{type:Object},config:{type:Object},expanded:{type:Boolean},_expanded:{type:Array,state:!0},_filters:{type:Array,state:!0},_entities:{type:Array,state:!0}};constructor(){super(),this.hass={},this.config={},this.expanded=!1,this._expanded=Array(5).fill(!1);const e=Object.keys(Pe);this._filters=Array(5).fill().map(()=>[...e]),this._entities=Array(5).fill(""),this._ignoreNextFilterChange=new Set}updated(e){if(e.has("config")||e.has("hass")){const e=fe(this.hass,this.config,"auto_discovery_sections.sensor");e&&e!==this.config&&this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0}));for(let e=0;e<5;e++){const t=`sensor${e+1}`,o=this.config?.sensor_filters?.[e],i=this.config?.entities?.[t]?.entity;Array.isArray(o)&&(this._filters[e]=[...o]),i&&(this._entities[e]=i)}}}static styles=[Me,n`
    ...
    `];render(){const e=this.config?.auto_discovery_sections?.sensor??!1,t=(e,t,o)=>Ae(this.hass,e,t,o),o=Object.entries(Pe).filter(([e])=>"_fallback"!==e).map(([e,t])=>{const o=t.label||e.replace(/_/g," ").replace(/\b\w/g,e=>e.toUpperCase());return{value:e,label:`${t.emoji||""} ${o}`.trim()}});return T`
      <ha-expansion-panel
        class="glass-panel"
        .expanded=${this.expanded}
      ...
      <div class="mini-pill ${t?"expanded":""}">
        ...
        ${t?T`
          <div class="mini-pill-content">
            ...
            ${n?(()=>{const e=this.hass.states[n],t=e?.attributes?.device_class,o=Pe[t]||{},i=o.emoji||"❓",s=e?.attributes?.unit_of_measurement||(o.units?.[0]??"");return T`
                <div class="preview">
    ...
    `];render(){const e=this.config?.auto_discovery_sections?.mushroom??!1,t=(e,t,o)=>Ae(this.hass,e,t,o),o=this._ALL_CATS.map(e=>({value:e,label:ne[e]||e.charAt(0).toUpperCase()+e.slice(1)}));return T`
      <ha-expansion-panel
        class="glass-panel"
        .expanded=${this.expanded}
        @expanded-changed=${e=>{this.expanded=e.detail.expanded,this.expanded&&(this._expanded=Array(5).fill(!1))}}
    ...
    `}_extraFields(e,t,o){const i=(e,t,o)=>Ae(this.hass,e,t,o),s=o?.[`${t}_action`]?.action;return"navigate"===s?T`
    ...
    `];render(){const e=this.config?.auto_discovery_sections?.subbutton??!1,t=(e,t,o)=>Ae(this.hass,e,t,o),o=ae.map(e=>({value:e,label:ne[e]||e.charAt(0).toUpperCase()+e.slice(1)}));return T`
      <ha-expansion-panel
        class="glass-panel"
        .expanded=${this.expanded}
        @expanded-changed=${e=>this.expanded=e.detail.expanded}
...
`;class Te extends ie{static properties={hass:{type:Object},config:{type:Object},expanded:{type:Boolean},_room:{type:Object,state:!0},_subbutton:{type:Object,state:!0},_mushroom:{type:Object,state:!0},_sensor:{type:Object,state:!0},_selectedPreset:{type:String,state:!0},_expandedColors:{type:Array,state:!0},_subbuttonStyle:{type:String,state:!0}};constructor(){super(),this.hass={},this.config={},this.expanded=!1,this._room={},this._subbutton={},this._mushroom={},this._sensor={},this._selectedPreset="green",this._expandedColors=[!1,!1,!1],this._subbuttonStyle="standard"}updated(e){if(e.has("config")){const e=this.config?.colors||{};this._room={icon_active:e.room?.icon_active??"",icon_inactive:e.room?.icon_inactive??"",background_active:e.room?.background_active??"",background_inactive:e.room?.background_inactive??"",text_active:e.room?.text_active??"",text_inactive:e.room?.text_inactive??""},this._subbutton={background_on:e.subbutton?.background_on??"",background_off:e.subbutton?.background_off??"",icon_on:e.subbutton?.icon_on??"",icon_off:e.subbutton?.icon_off??""},this._mushroom={active:e.mushroom?.active??"",inactive:e.mushroom?.inactive??""},this._sensor={sensor_active:e.sensor?.sensor_active??"",sensor_inactive:e.sensor?.sensor_inactive??""},this._subbuttonStyle=this.config?.subbutton_style||"standard"}}get PRESETS(){return Re}static styles=je;render(){const e=(e,t,o)=>Ae(this.hass,e,t,o);return T`
    ...
    `}_renderStyleChooser(){const e=(e,t,o)=>Ae(this.hass,e,t,o),t=[{key:"standard",label:e("panel.colors.style.standard"),description:e("panel.colors.style.standard_desc")},{key:"liquid-glass",label:e("panel.colors.style.liquid_glass"),description:e("panel.colors.style.liquid_glass_desc")}];return T`
    ...
    `}_renderStyleCard({key:e,label:t,description:o}){const i=this._subbuttonStyle===e;return T`
      <div
    ...
    `}_renderPresetChooser(){const e=Object.keys(this.PRESETS);return T`
      <div class="preset-bar">
        ${e.map(e=>this._renderPresetCard(e,this.PRESETS[e]))}
      </div>
      <div class="apply-row">
        <button class="apply-btn" @click=${this._applySelectedPreset}>
          ${((e,t,o)=>Ae(this.hass,e,t,o))("panel.colors.apply_preset")}
        </button>
      </div>
    `}_renderPresetCard(e,t){const o=(e,t,o)=>Ae(this.hass,e,t,o),i=this._selectedPreset===e?"selected":"",s=t.room.background_active,n=t.room.background_inactive,a=t.room.icon_active,r=t.room.icon_inactive;return T`
    ...
    `}_toggleColor(e){this._expandedColors=this._expandedColors.map((t,o)=>o===e&&!t)}_renderColorField(e,t,o){const i=this.config?.colors?.[e]?.[t]||"",[s,n,a,r]=this._parseRGBA(i),l=`#${[s,n,a].map(e=>e.toString(16).padStart(2,"0")).join("")}`;return T`
      <div class="input-group">
        <label>${o}</label>
        <input
          type="color"
  ...
  `;render(){const e=this.config?.auto_discovery_sections?.camera??!1,t=(e,t,o)=>Ae(this.hass,e,t,o);return T`
      <ha-expansion-panel
        class="glass-panel"
        .expanded=${this.expanded}
        @expanded-changed=${e=>this.expanded=e.detail.expanded}
  ...
  `;render(){const e=this.config?.auto_discovery_sections?.climate??!1,t=(e,t,o)=>Ae(this.hass,e,t,o);return T`
      <ha-expansion-panel
        class="glass-panel"
        .expanded=${this.expanded}
        @expanded-changed=${e=>this.expanded=e.detail.expanded}
  ...
  `;render(){const e=this.config||{};return T`
    ...
    `}}customElements.define("bubble-room-editor",De);var Be=Object.freeze({__proto__:null,BubbleRoomEditor:De});function He({holdThreshold:e=500,onTap:t,onHold:o}={}){let i,s=null,n=!1;function a(){null!==s&&(clearTimeout(s),s=null)}return{onDown:function(t){n=!1,i=t,a(),s=setTimeout(()=>{n=!0,s=null,o?.(i)},e)},onUp:function(e){a(),n?n=!1:void 0!==e&&void 0!==i&&e!==i||t?.(void 0!==e?e:i)},onCancel:function(){a(),n=!1,i=void 0},clearTimer:a}}let Ue=null,Le=null;function We(e){if(!e||"string"!=typeof e||e.startsWith("var("))return null;const t=Le||("undefined"==typeof document?null:(Ue=document.createElement("canvas"),Ue.width=Ue.height=1,Le=Ue.getContext("2d",{willReadFrequently:!0})||Ue.getContext("2d"),Le));if(!t)return null;try{t.fillStyle="#000",t.fillStyle=e}catch{return null}const o=t.fillStyle;t.clearRect(0,0,1,1),t.fillStyle=o,t.fillRect(0,0,1,1);const[i,s,n,a]=t.getImageData(0,0,1,1).data;return{r:i,g:s,b:n,a:a/255}}class Ve extends ie{static properties={subbuttons:{type:Array},preset:{type:String,reflect:!0}};constructor(){super(),this.subbuttons=[],this.preset="liquid-glass",this._gesture=He({onTap:e=>this._fireHassAction(e,"tap"),onHold:e=>this._fireHassAction(e,"hold")})}static styles=n`
    :host {
      display: block;
      height: 100%;
      width: 100%;
  ...
  `;render(){return"standard"===this.preset?this._renderStandard():this._renderLiquidGlass()}_renderStandard(){return T`
      <div class="container">
        ${this.subbuttons.map((e,t)=>{const o=e.active?e.colorOn:e.colorOff,i=e.active?e.iconOn:e.iconOff;return T`
            <div
              class="sub-button"
  ...
  `;render(){const e=(this.sensors||[]).map(e=>{const t=e.device_class,o=Pe[t]||{},i=o.emoji||"❓",s=e.unit||o.units?.[0]||"",n=this._formatValueForDisplay(e.value,1,!0);return{...e,value:n,label:i,unit:s}});return T`
      <div
        class="sensor-grid"
        style="
        ...
        "
      >
        ${e.map(e=>{const t=e.entity||e.entity_id||"",o=t?`Show history graph: ${t}`:"Show history graph";return T`
            <div
              class="sensor-pill"
  ...
  `;render(){const{width:e,height:t}=this._containerSize;if(!e||!t)return T``;const o=window.innerWidth||e,i=.55;let s;if(o<=100)s=i;else if(o>=200)s=.25;else{s=i+(.25-i)*((o-100)/100)}const n=Math.min(e,1.6*t),a=.5*(t+n)*s,r=.6*e,l=.6*t,c=r*Math.min(1,e/(2*r)),d=l*Math.min(1,t/(2*l)),u=e-c,p=.5*t,h=Math.max(0,c-a/2-1),b=Math.max(0,d-a/2-1),m=e=>Math.PI*e/180,g=m(30),_=m(85),f=.75*a,v=.75*a,y=[{x:a/2+1,y:a/2+1},{x:u+h*Math.cos(-_),y:p+b*Math.sin(-_)},{x:u+h*Math.cos(-g),y:p+b*Math.sin(-g)},{x:u+h*Math.cos(+g),y:p+b*Math.sin(+g)},{x:u+h*Math.cos(+_),y:p+b*Math.sin(+_)}];let x=0;return T`
      ${this.entities.map(o=>{const i="camera"===o?.kind,s="climate"===o?.kind,n=i?f:s?v:a,r=.95*n;let l;i?l={x:e-n/2,y:n/2}:s?l={x:n/2+1,y:t-n/2-1}:(l=y[Math.min(x,y.length-1)]??{x:u,y:p},x++);const c=i||s?0:o.dx??0,d=i||s?0:o.dy??0,h=l.x+c,b=l.y+d,m=o.entity_id||o.kind||"unknown",g=this._ripplingKeys.has(m);return T`
          <div
          ...
          >
            ${g?T`<span class="ripple"></span>`:""}
            <ha-icon icon="${o.icon||ke(o.entity_id,this.hass)}" style="--mdc-icon-size:${r}px;"></ha-icon>
          </div>
        `})}
    `}}customElements.define("bubble-mushroom",Ke);class Ye extends ie{static properties={icon:{type:String},active:{type:Boolean},colorActive:{type:String},colorInactive:{type:String},backgroundActive:{type:String},backgroundInactive:{type:String},preset:{type:String,reflect:!0},entity_id:{type:String},tap_action:{type:Object},hold_action:{type:Object}};constructor(){super(),this.icon="",this.active=!1,this.colorActive="#21df73",this.colorInactive="#173c16",this.backgroundActive="rgba(33,223,115,0.1)",this.backgroundInactive="rgba(23,60,22,0.1)",this.preset="standard",this.entity_id="",this.tap_action={action:"more-info"},this.hold_action={action:"none"},this._gesture=He({onTap:()=>this._fireHassAction("tap"),onHold:()=>this._fireHassAction("hold")})}static styles=n`
    :host {
      position: absolute;
      display: block;
      inset: 0;
  ...
  `;render(){const e=this.active?this.colorActive:this.colorInactive,t=this.active?this.backgroundActive:this.backgroundInactive,o="liquid-glass"===this.preset,i=this._withOpacity(t,o?.22:.1)??t,s=this.active?.9:.8,n=[];i&&n.push(`--bubble-main-icon-bg:${i}`),e&&(n.push(`color:${e}`),o||n.push(`--bubble-main-icon-border:${e}`),n.push(`--bubble-main-icon-color:${e}`));const a=n.map(e=>`${e};`).join(" ");return T`
      <div
        class="container"
        style="${a}"
    ...
    `}_withOpacity(e,t){return function(e,t){const o=We(e);return o?`rgba(${o.r}, ${o.g}, ${o.b}, ${t})`:null}(e,t)}_onDown=()=>this._gesture.onDown();_onUp=()=>this._gesture.onUp();_clearHoldTimer=()=>this._gesture.clearTimer();_fireHassAction(e){const t=(("hold"===e?this.hold_action:this.tap_action)||{action:"more-info"}).action||"more-info";if(("toggle"===t||"call-service"===t||"more-info"===t)&&!this.entity_id)return;const o=new Event("hass-action",{bubbles:!0,composed:!0});o.detail={config:{entity:this.entity_id,tap_action:this.tap_action||{action:"more-info"},hold_action:this.hold_action||{action:"none"}},action:e},this.dispatchEvent(o)}}customElements.define("bubble-icon",Ye);class Ze extends ie{static properties={config:{type:Object},hass:{type:Object}};_entities={};constructor(){super(),this.config={},this.hass={}}setConfig(e){this.config={layout:"wide",...e},this._entities=structuredClone(this.config.entities||{}),this._entities.camera=this._entities.camera||{entity:"",icon:""},this._entities.camera.presence||(this._entities.camera.presence={entity:""}),this._entities.climate=this._entities.climate||{entity:"",icon:""}}get hass(){return this._hass}set hass(e){this._hass=e,e?.states&&this.requestUpdate?.()}static getStubConfig(){return{type:"custom:bubble-room",layout:"wide",name:[],area:[],sensors:[],mushrooms:[],subbuttons:[],colors:{subbutton:{background_on:"rgba(var(--color-blue),1)",background_off:"rgba(var(--color-blue),0.3)",icon_on:"yellow",icon_off:"#666"}}}}static async getConfigElement(){return await Promise.resolve().then(function(){return Be}),document.createElement("bubble-room-editor")}connectedCallback(){super.connectedCallback(),this._resizeObs=new ResizeObserver(()=>this.requestUpdate())}firstUpdated(){const e=this.shadowRoot?.querySelector(".icon-mushroom-area");e&&this._resizeObs.observe(e)}disconnectedCallback(){this._resizeObs?.disconnect(),super.disconnectedCallback()}updated(e){e.has("config")&&(this._entities=structuredClone(this.config.entities||{}))}_getSubButtons(){const e=this.config.colors?.subbutton?.background_on??"#00d46d",t=this.config.colors?.subbutton?.background_off??"#999",o=this.config.colors?.subbutton?.icon_on??"yellow",i=this.config.colors?.subbutton?.icon_off??"#666";return(this.config.subbuttons||[]).map(s=>{const n=this.hass.states?.[s.entity_id];return{icon:s.icon||ke(s.entity_id,this.hass),active:"on"===n?.state,colorOn:e,colorOff:t,iconOn:o,iconOff:i,entity_id:s.entity_id,tap_action:s.tap_action,hold_action:s.hold_action}})}_isRoomActive(){const e=this.config?.entities?.presence?.entity;if(!e)return!1;const t=this.hass?.states?.[e]?.state;return["on","home","occupied","motion","detected"].includes(t)}_getMainIconSize(){const e=this.shadowRoot?.querySelector(".icon-mushroom-area");return e?Math.round(.6*Math.min(e.clientWidth,e.clientHeight)):64}_getSensors(){const e=this._entities||{},t=this.config.colors?.room?.sensor_active??this.config.colors?.room?.text_active??"#21df73",o=this.config.colors?.room?.sensor_inactive??this.config.colors?.room?.text_inactive??"#173c16",i=this._isRoomActive()?t:o,s=[];for(let t=1;t<=6;t++){const o=e[`sensor${t}`]?.entity,n=this.hass?.states?.[o];o&&n&&s.push({icon:n.attributes.icon||"",value:n.state,unit:n.attributes.unit_of_measurement,device_class:n.attributes.device_class,color:i,entity:o})}return s}_getMushrooms(){const e=this._entities||{},t=this.config.colors?.mushroom?.active??"#00e676",o=this.config.colors?.mushroom?.inactive??"#888",i=[];for(let s=1;s<=5;s++){const n=e[`mushroom${s}`]||{},a=n.entity,r=this.hass?.states?.[a];a&&r&&i.push({icon:n.icon||r.attributes.icon||ke(a,this.hass)||"mdi:flash",state:r.state,color:"on"===r.state?t:o,dx:n.dx??0,dy:n.dy??0,angle_deg:n.angle_deg,radius_factor:n.radius_factor,entity_id:a,tap_action:n.tap_action,hold_action:n.hold_action})}const s=e.camera||{},n=s.entity;if(n&&this.hass.states?.[n]){const e=this.hass?.states?.[n],a=s.presence?.entity,r=a?this.hass?.states?.[a]?.state:void 0,l=!a||["on","home","occupied","motion","detected"].includes(r);i.push({icon:s.icon||e.attributes.icon||ke(n,this.hass)||"mdi:cctv",state:e.state,color:l?t:o,left:"calc(100% - 12px - 36px)",top:12,dx:0,dy:0,kind:"camera",angle_deg:s.angle_deg,radius_factor:s.radius_factor,entity_id:n,tap_action:{action:"more-info"},hold_action:{action:"none"}})}const a=this._entities?.climate||{},r=a.entity;if(r&&this.hass.states?.[r]){const e=this.hass?.states?.[r],s=e.state&&"off"!==e.state&&"idle"!==e.state||e.attributes?.hvac_action&&"off"!==e.attributes.hvac_action;i.push({icon:a.icon||e.attributes.icon||ke(r,this.hass)||"mdi:thermostat",state:e.state,color:s?t:o,dx:0,dy:0,angle_deg:a.angle_deg,radius_factor:a.radius_factor,kind:"climate",entity_id:r})}return i}_onMushroomClick(e){}render(){try{return this._renderCard()}catch(e){return console.error("[bubble-room] render error:",e),T`<div style="padding:12px;color:#f87;font-size:0.9rem">
        bubble-room: errore di rendering — ${e.message}
      </div>`}}_renderCard(){const e=this.config.layout||"wide",t=this._getMainIconSize(),o=this._getSubButtons(),i=this.config?.subbutton_style||"standard",s=this._isRoomActive(),n=this.config.colors?.room?.icon_active??"#21df73",a=this.config.colors?.room?.icon_inactive??"#173c16",r=this.config.colors?.room?.background_active??"rgba(33,223,115,0.12)",l=this.config.colors?.room?.background_inactive??"rgba(23,60,22,0.12)",c=this.config.colors?.room?.text_active??"#ffffff",d=this.config.colors?.room?.text_inactive??"rgba(255,255,255,0.5)",u=this.config?.entities?.presence?.entity||"",p=this.config?.tap_action||{action:"more-info"},h=this.config?.hold_action||{action:"none"};return T`
