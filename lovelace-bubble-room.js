...
`;class Oe extends ie{static properties={hass:{type:Object},config:{type:Object},expanded:{type:Boolean},_expanded:{type:Array,state:!0},_filters:{type:Array,state:!0},_entities:{type:Array,state:!0}};constructor(){super(),this.hass={},this.config={},this.expanded=!1,this._expanded=Array(5).fill(!1);const e=Object.keys(Pe);this._filters=Array(5).fill().map(()=>[...e]),this._entities=Array(5).fill(""),this._ignoreNextFilterChange=new Set}updated(e){if(e.has("config")||e.has("hass")){const e=_e(this.hass,this.config,"auto_discovery_sections.sensor");e&&e!==this.config&&this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:e},bubbles:!0,composed:!0}));for(let e=0;e<5;e++){const t=`sensor${e+1}`,o=this.config?.sensor_filters?.[e],i=this.config?.entities?.[t]?.entity;Array.isArray(o)&&(this._filters[e]=[...o]),i&&(this._entities[e]=i)}}}static styles=[Fe,n`
      :host {
    ...
    `];render(){const e=this.config?.auto_discovery_sections?.sensor??!1,t=(e,t,o)=>Ee(this.hass,e,t,o),o=Object.entries(Pe).filter(([e])=>"_fallback"!==e).map(([e,t])=>{const o=t.label||e.replace(/_/g," ").replace(/\b\w/g,e=>e.toUpperCase());return{value:e,label:`${t.emoji||""} ${o}`.trim()}});return j`
      <ha-expansion-panel
        class="glass-panel"
        .expanded=${this.expanded}
...
`;class Ne extends ie{static properties={hass:{type:Object},config:{type:Object},expanded:{type:Boolean},_room:{type:Object,state:!0},_subbutton:{type:Object,state:!0},_mushroom:{type:Object,state:!0},_sensor:{type:Object,state:!0},_cardBg:{type:Object,state:!0},_selectedPreset:{type:String,state:!0},_expandedColors:{type:Array,state:!0},_subbuttonStyle:{type:String,state:!0}};constructor(){super(),this.hass={},this.config={},this.expanded=!1,this._room={},this._subbutton={},this._mushroom={},this._sensor={},this._cardBg={enabled:!1,color:""},this._selectedPreset="green",this._expandedColors=[!1,!1,!1,!1,!1],this._subbuttonStyle="standard"}updated(e){if(e.has("config")){const e=this.config?.colors||{};this._room={icon_active:e.room?.icon_active??"",icon_inactive:e.room?.icon_inactive??"",background_active:e.room?.background_active??"",background_inactive:e.room?.background_inactive??"",text_active:e.room?.text_active??"",text_inactive:e.room?.text_inactive??""},this._subbutton={background_on:e.subbutton?.background_on??"",background_off:e.subbutton?.background_off??"",icon_on:e.subbutton?.icon_on??"",icon_off:e.subbutton?.icon_off??""},this._mushroom={active:e.mushroom?.active??"",inactive:e.mushroom?.inactive??""},this._sensor={sensor_active:e.sensor?.sensor_active??"",sensor_inactive:e.sensor?.sensor_inactive??""},this._subbuttonStyle=this.config?.subbutton_style||"standard",this._cardBg={enabled:this.config?.card_background?.enabled??!1,color:this.config?.card_background?.color??""}}}get PRESETS(){return Ie}static styles=je;render(){const e=(e,t,o)=>Ee(this.hass,e,t,o);return j`
      <ha-expansion-panel
        class="glass-panel"
        .expanded=${this.expanded}
