...
`;class Mt extends st{static properties={hass:{type:Object},config:{type:Object},expanded:{type:Boolean},_expanded:{type:Array,state:!0},_filters:{type:Array,state:!0},_entities:{type:Array,state:!0}};constructor(){super(),this.hass={},this.config={},this.expanded=!1,this._expanded=Array(5).fill(!1);const t=Object.keys(zt);this._filters=Array(5).fill().map(()=>[...t]),this._entities=Array(5).fill(""),this._ignoreNextFilterChange=new Set}updated(t){if(t.has("config")||t.has("hass")){const t=_t(this.hass,this.config,"auto_discovery_sections.sensor");t&&t!==this.config&&this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0}));for(let t=0;t<5;t++){const e=`sensor${t+1}`,i=this.config?.sensor_filters?.[t],s=this.config?.entities?.[e]?.entity;Array.isArray(i)&&(this._filters[t]=[...i]),s&&(this._entities[t]=s)}}}static styles=[Pt,o`
  ...
  `;render(){return N`
    ...
    `}_toggleColor(t){this._expandedColors=this._expandedColors.map((e,i)=>i===t&&!e)}_renderColorField(t,e,i){const s=this.config?.colors?.[t]?.[e]||"",[n,o,a,r]=this._parseRGBA(s),l=`#${[n,o,a].map(t=>t.toString(16).padStart(2,"0")).join("")}`;return N`
      <div class="input-group">
        <label>${i}</label>
        <input
          type="color"
  ...
  `;render(){const t=this.config||{};return N`
    ...
    .sub-button {
      ...
      background:
        radial-gradient(130% 160% at 18% 12%, var(--bubble-subbutton-glass-sheen, rgba(255, 255, 255, 0.38)), rgba(255, 255, 255, 0) 72%),
        radial-gradient(125% 165% at 84% -10%, rgba(255, 255, 255, 0.22), rgba(255, 255, 255, 0) 74%),
        linear-gradient(148deg, var(--bubble-subbutton-glass-highlight, rgba(255, 255, 255, 0.24)), rgba(255, 255, 255, 0.08) 44%, rgba(255, 255, 255, 0) 78%),
        linear-gradient(205deg, var(--bubble-subbutton-glass-soft, rgba(var(--bubble-subbutton-tint, 255, 255, 255), 0.14)), rgba(var(--bubble-subbutton-tint, 255, 255, 255), 0.06) 52%, rgba(var(--bubble-subbutton-tint, 255, 255, 255), 0.22) 100%),
      ...
      box-shadow:
        ...
        0 0 0 1px var(--bubble-subbutton-glass-halo, rgba(var(--bubble-subbutton-tint, 255, 255, 255), 0.12)),
        0 10px 22px var(--bubble-subbutton-glass-shadow, rgba(13, 22, 41, 0.2));
    ...
    .sub-button:hover {
      box-shadow:
        ...
        0 0 0 1px var(--bubble-subbutton-glass-halo-hover, rgba(var(--bubble-subbutton-tint, 255, 255, 255), 0.18)),
        0 14px 26px var(--bubble-subbutton-glass-shadow-hover, rgba(13, 22, 41, 0.26));
    }
  ...
  `;render(){const{width:t,height:e}=this._containerSize;if(!t||!e)return N``;const i=window.innerWidth||t,s=.55;let n;if(i<=100)n=s;else if(i>=200)n=.25;else{n=s+(.25-s)*((i-100)/100)}const o=Math.min(t,1.6*e),a=.5*(e+o)*n,r=.6*t,l=.6*e,c=r*Math.min(1,t/(2*r)),h=l*Math.min(1,e/(2*l)),d=t-c,u=.5*e,p=Math.max(0,c-a/2-1),b=Math.max(0,h-a/2-1),g=t=>Math.PI*t/180,f=g(30),m=g(85),_=.75*a,v=.75*a,y=[{x:a/2+1,y:a/2+1},{x:d+p*Math.cos(-m),y:u+b*Math.sin(-m)},{x:d+p*Math.cos(-f),y:u+b*Math.sin(-f)},{x:d+p*Math.cos(+f),y:u+b*Math.sin(+f)},{x:d+p*Math.cos(+m),y:u+b*Math.sin(+m)}];let x=0;return N`
      ${this.entities.map(i=>{const s="camera"===i?.kind,n="climate"===i?.kind,o=s?_:n?v:a,r=.95*o;let l;s?l={x:t-o/2,y:o/2}:n?l={x:o/2+1,y:e-o/2-1}:(l=y[Math.min(x,y.length-1)]??{x:d,y:u},x++);const c=s||n?0:i.dx??0,h=s||n?0:i.dy??0,p=l.x+c,b=l.y+h;return N`
          <div
