...
export class BubbleSubButton extends LitElement {
  ...
  _computeGlassColors(color) {
    ...
    const base = this._formatRgba(this._mixRgb(rgb, white, 0.3), 0.05 + alpha * 0.05);
    const highlight = this._formatRgba(this._mixRgb(rgb, white, 0.88), 0.32);
    const soft = this._formatRgba(this._mixRgb(rgb, white, 0.64), 0.18);
    const sheen = this._formatRgba(this._mixRgb(rgb, white, 0.92), 0.45);
    const accent = this._formatRgba(this._mixRgb(rgb, white, 0.48), 0.16);
    const glow = this._formatRgba(this._mixRgb(rgb, white, 0.4), 0.14);
    const halo = this._formatRgba(this._mixRgb(rgb, white, 0.3), 0.12 + alpha * 0.06);
    const haloHover = this._formatRgba(this._mixRgb(rgb, white, 0.3), 0.18 + alpha * 0.08);
    const haloActive = this._formatRgba(this._mixRgb(rgb, white, 0.3), 0.1 + alpha * 0.05);

    const shadowMix = this._mixRgb(rgb, depth, 0.6);
    const shadowRgb = `${shadowMix.r}, ${shadowMix.g}, ${shadowMix.b}`;

    const shadow = this._formatRgba(shadowMix, 0.2);
    const shadowHover = this._formatRgba(shadowMix, 0.24);
    const shadowActive = this._formatRgba(shadowMix, 0.18);
