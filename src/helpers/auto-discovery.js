 ...
 *   TRIGGER CENTRALE
 * ========================= */
export function maybeAutoDiscover(hass, config, changedProp, debug = false) import { candidatesFor } from './entity-filters.js';
...
// Climate (NUOVO con filtro area)
export function autoFillClimate(hass, config) {
  const entities = { ...(config.entities || {}) };
  const climate = entities.climate || (entities.climate = {});
  if (climate.entity) return { ...config, entities };

  const candidates = candidatesFor(hass, config, 'climate') || [];
  const pick = candidates[0];
  if (pick) climate.entity = pick;
  return { ...config, entities };
}
